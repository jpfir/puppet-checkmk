  ...
  case $facts['os']['family'] {
    ...
    'RedHat': {
      if $checkmk::download_url {
        ...
        $download_url = "https://download.checkmk.com/checkmk/${checkmk::version}/check-mk-raw-${checkmk::version}.el${facts['os']['release']['major']}.x86_64.rpm"
      }

      file { '/tmp/check-mk-raw.rpm':
        ensure         => file,
        source         => $download_url,
        checksum       => 'sha256',
        checksum_value => $checkmk::sha256_hash,
      ...
      provider => 'rpm',
      source   => '/tmp/check-mk-raw.rpm',
      require  => File['/tmp/check-mk-raw.rpm'],
      }
    }
    default: {
